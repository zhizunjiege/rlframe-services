...
class ComplexReplay:
    ...
    def _encode_sample(self, batch_size: int) -> Dict[str, np.ndarray]:
        idxes = [random.randint(0, self.size - 1) for _ in range(batch_size)]
        obses_t, actions, rewards, obses_tp1, dones = [], [], [], [], []
        for i in idxes:
            obses_t.append(np.array(self.obs1_buf[i], copy=False))
            actions.append(np.array(self.acts_buf[i], copy=False))
            rewards.append(self.rews_buf[i])
            obses_tp1.append(np.array(self.obs2_buf[i], copy=False))
            dones.append(self.term_buf[i])
        #return np.array(obses_t), np.array(actions), np.array(rewards), np.array(obses_tp1), np.array(dones)
        return dict(
            obs1=np.array(obses_t),
            obs2=np.array(obses_tp1),
